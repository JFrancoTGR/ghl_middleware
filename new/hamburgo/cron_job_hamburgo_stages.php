<?php
// ============================================
// CAPISoft -> GHL Stage Sync (HAMBURGO) [PRODUCTIVO V1]
// - Open stages only
...
$RUNTIME_PROJECT_ID       = 13;
$RUNTIME_SINCE_DATE       = '2026-06-01';
...
$GHL_PIPELINE_ID          = 'oUHz8QHC9rIw0J7QnNwa';
$GHL_CF_CAPISOFT_STAGE_ID = '9akn1HKwx4LzwwKING1w';

$CAPI_TO_GHL_STAGE = [
    183 => '0fb9f300-3344-46f9-b7cc-fac625738a6f', // Asignado
    237 => '8930f448-df87-406a-8394-5d0efe196811', // Buscando contacto CAPI
    347 => 'de1938af-4e2b-4067-b437-ce263ffc4821', // Enfriado
    348 => 'd6a9c9df-92b6-4473-9f29-782accb86a3b', // Nurture
    184 => '9b9bff32-1cce-470e-b457-d0c5518a52dd', // Seguimiento
    349 => 'baf61b07-a403-446d-a3d3-3fc1aa4d24d9', // Negociación
...
if (! in_array($logMode, ['normal', 'debug'], true)) {
    $logMode = 'normal';
}
...
$stateFile         = $storageDir . '/capisoft_state_project_13_stage_productive.json';
$progressFile      = $storageDir . '/capisoft_progress_project_13_stage_productive.json';
$lockFile          = $storageDir . '/capisoft_shared_project_13.lock';
$logFile           = $logsDir . '/capisoft_stage_hamburgo_productive.log';
$lockSkipFile      = $logsDir . '/stage_lock_skips.log';
$ownerFixTraceFile = $logsDir . '/capisoft_stage_hamburgo_ownerfix_trace.log';
...
function extract_attempt_http(array $lookup, string $channel): ?int
{
    foreach (array_reverse($lookup['attempts'] ?? []) as $a) {
        if (($a['type'] ?? null) === $channel) {
            return isset($a['http']) ? (int) $a['http'] : null;
        }
...
function extract_attempt_count(array $lookup, string $channel): ?int
{
    foreach (array_reverse($lookup['attempts'] ?? []) as $a) {
        if (($a['type'] ?? null) === $channel) {
            return isset($a['count']) ? (int) $a['count'] : null;
        }
...
function http_json(string $method, string $url, array $headers, $payload = null, int $timeout = 25): array
{
    ...
    if (! empty($GLOBALS['api_trace_enabled']) && is_debug_mode()) {
        ...
        if ($err !== '') {
            $line .= ' curl_err=' . one_line($err, 200);
        }

        if (! empty($GLOBALS['api_trace_bodies_enabled'])) {
            if ($requestBody !== null) {
                $line .= ' req=' . one_line($requestBody, (int) $GLOBALS['api_trace_max_chars']);
            }

            if ($body !== false && $body !== null && $body !== '') {
                $line .= ' resp=' . one_line($body, (int) $GLOBALS['api_trace_max_chars']);
            }
...
function is_rate_limited(array $resp): bool
{return ((int) ($resp['http'] ?? 0) === 429);}
...
function capi_fetch_snapshot(int $projectId, string $sinceDate): array
{
    ...
    if ((int) $resp['http'] >= 400 || ! is_array($resp['json'])) {
        return ['ok' => false, 'resp' => $resp, 'rows' => []];
    }
...
function ghl_get_contact(string $contactId): array
{
    ...
    $contact = null;
    if ((int) $resp['http'] < 400 && is_array($resp['json'])) {
        if (isset($resp['json']['contact']) && is_array($resp['json']['contact'])) {
            $contact = $resp['json']['contact'];
        } elseif (isset($resp['json']['id'])) {
            $contact = $resp['json'];
        }
...
{
    $opps = [];
    if (isset($contact['opportunities']) && is_array($contact['opportunities'])) {
...
while ($idx < $totalFiltered) {
    ...
    if ($targetOwnerId && $currentContactOwner === '') {
        $needsContactOwner = true;
    }
